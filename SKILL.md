...
7. **Name any scaffolded repo after the task** — if the task involves creating a new app or repo, use a `fl-` prefix followed by the feature or API being tested (e.g. `fl-catcherror`, `fl-instant-nav`). The prefix makes clear the repo was created as part of a friction log run. Do not use generic names like `my-app` or `test-app`. Write the friction log file **inside** this folder (`fl-<feature>/friction-log.md`), not in the parent directory.
